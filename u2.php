...
if($_POST){
    ...
    if(empty($nama_pegawai)){
        ...
        if(empty($password)){
            ...
            $update=mysqli_query($conn,"update tabel_pegawai set nik_pegawai='".$nik_pegawai."',nama_pegawai='".$nama_pegawai."',alamat='".$alamat."', jenis_kelamin='".$jenis_kelamin."', no_telp='".$no_telp."', id_jabatan='".$id_jabatan."', username='".$username."', password='".md5($password)."', where id_pegawai = '".$id_pegawai."'") or die(mysqli_error($conn));
            if($update){
                echo "<script>alert('Sukses update pegawai');location.href='tampil_pegawai.php';</script>";
            } else {
                echo "<script>alert('Gagal update pegawai');location.href='ubah_pegawai.php?id_pegawai=".$id_pegawai."';</script>";
